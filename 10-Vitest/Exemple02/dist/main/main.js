...
class ParticipantService {
  ...
  async chargerParticipants() {
    try {
      ...
      return rows.map((rows2) => new Participant({
        matricule: rows2.Matricule,
        prenom: rows2.Prenom,
        nom: rows2.Nom,
        genre: rows2.Genre,
        niveau: rows2.Niveau,
        email: rows2.Email,
        isActif: rows2.IsActif === 1 ? true : false
      }));
    } catch (error2) {
      console.log("Erreur lors du chargment des paticipants: ", error2.message);
      throw error2;
    }
  }
  ...
  async ajouterParticipant(participant) {
    try {
      const query2 = `INSERT INTO participant (Matricule, Prenom, Nom, Genre, Niveau, Email, IsActif)
      VALUES (?,?,?,?,?,?,?)`;
      const values = [
      ...
      await this.db.execute(query2, values);
    } catch (error2) {
      console.log("Erreur lors de l'ajout d'un paticipants: ", error2.message);
      throw error2;
    }
  }
  ...
  async supprimerParticipant(matricule) {
    try {
      ...
      console.log("Erreur lors de l'a supression d'un paticipants: ", error2.message);
      throw error2;
    }
  }
  ...
  async modifierParticipant(updated) {
    try {
      ...
      const query2 = `UPDATE participant
      ...
      const result = await this.db.execute(query2, values);
      if (result.affectedRows == 0) {
        throw new Error(`Participant avec matricule ${updated.matricule} introubale`);
      }
    } catch (error2) {
      console.log("Erreur lors de l'a modification du paticipants: ", error2.message);
      throw error2;
    }
  }
...
electron.app.on("ready", async () => {
  ...
  try {
    ...
    electron.dialog.showErrorBox("Erreur de connexion", "Impossible de se connecter à la BD Vérifier que Mysql est en cours d'éxécution");
    electron.app.quit();
    return;
  }
