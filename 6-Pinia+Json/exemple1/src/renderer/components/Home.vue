<template>
  <v-app>
    ...
    <v-navigation-drawer app v-model="drawer" temporary width="200" color="green lighten-4" elevation="10">
      ...
      <v-list>
        <v-list-item title="Ajouter" prepend-icon="mdi-home" @click="ouvrirAjouterParticipant"></v-list-item>
        <v-list-item title="Supprimer" prepend-icon="mdi-home" @click="supprimerParticipant"></v-list-item>
      </v-list>
    </v-navigation-drawer>
      ...
      <v-container>
        <v-data-table :items="participants" :headers="headers" item-value="matricule" class="lignes-alternance"
          ...
          <template v-slot:item.niveau="{ item }">
            <v-select v-model="item.niveau" :items="['Débutant', 'Intermédiaire', 'Professionnel']" density="compact"
              variant="outlined" hide-details disabled class="disabled-black">
...
//Pour contenire les information du participant à supprimer 
...
onMounted(async () => {
  ...
  // Configurer les écouteurs dIPC les chargement
