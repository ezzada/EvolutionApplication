...
export const useParticipantStore = defineStore('participant', () => {
    ...
    const participantsParNiveau = computed(() => {
        return (niveau) => participants.value.filter(p => p.niveau === niveau);
    });
    const participantsParMatricule = computed(() => {
        return (matricule) => participants.value.find(p => p.matricule === matricule);
    });
    ...
    async function ajouterParticipant(participant) {
        isLoading.value = true;
        error.value = null;
        try {
    ...
    function setupIpcListeners() {
        window.api.on('participant-added', (event, participant) => {
    ...
    async function supprimerParticipant(matricule) {
        isLoading.value = true;
        error.value = null;
        try {
