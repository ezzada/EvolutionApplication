<template>
  <v-app>
    ...
    <v-navigation-drawer app v-model="drawer" temporary width="200" color="green lighten-4" elevation="10">
      ...
      <v-list>
        <v-list-item title="Ajouter" prepend-icon="mdi-plus" @click="ouvrirAjouterParticipant" :diabled="tab !== 'table'"></v-list-item>
        <v-list-item title="Supprimer" prepend-icon="mdi-delete" @click="supprimerParticipant" :diabled="tab !== 'table'"></v-list-item>
        <v-list-item title="Modifier" prepend-icon="mdi-pencil" @click="ouvrirModifierParticipant" :diabled="tab !== 'table'"></v-list-item>
      </v-list>
    </v-navigation-drawer>
      ...
      <v-container>
        ...
        <v-window v-model="tab">
          <v-window-item value="table">
            <v-data-table :items="participants" :headers="headers" item-value="matricule" class="lignes-alternance"
              show-expand @click:row="(event: MouseEvent, row: { item: Participant }) => handleRowClick(row.item)">
              <!-- Afficher la colonne Nivau sous forme d'une liste déroulante désactivée -->
              <template v-slot:item.niveau="{ item }">
                <v-select v-model="item.niveau" :items="['Debutant', 'Intermediaire', 'Professionnel']"
                  density="compact" variant="outlined" hide-details disabled class="disabled-black">
  ...
  if (participant && participant.matricule) {
    ...
    if (result?.success) {
      await window.api.showMessageBox({
        type: "info",
        title: "Suppression",
        message: `Participant ${participant.nom} supprimer avec succès.`,
      });
    } else {
      await window.api.showMessageBox({
        type: "error",
